...
A feature slice has one structure in this repository, and `pnpm generate feature` emits it: a Zod contract and its invariants in `packages/domain` reporting stable codes an interface translates, a consumer-owned port in `packages/api`, a `groupProcedure` router that takes no group identifier, a Prisma adapter in `packages/db`, constraints in PostgreSQL that the application cannot write around, and web and native screens over layered tests. The committed `announcement` slice is that generator's output and is kept so by a drift test — over the files it creates and over the blocks it leaves in files it shares — so it is the worked example to read; the generator is the thing to run.

Structure is all a generator can carry across domains, and _meaning_ is what `--shape` stops it carrying by accident. The two shapes differ in how records relate to each other: under `current` one record per group is current, creating supersedes it, the repository does both writes in one transaction and a partial unique index refuses a concurrent second; under `list` records accumulate, creating is a single write, and there is no flag to keep consistent. Each has its own copy, so a product only ever reads a sentence about superseding when it asked for the shape that supersedes. The argument is mandatory precisely because a default would be the old imposition wearing an argument's clothes.
