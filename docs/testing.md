...
`pnpm verify` ends with `test:e2e:mobile`, and on a GitHub-hosted runner that step skips: a native journey needs an app build and a simulator neither the runner nor a Mac carrying only the command-line tools has. It says so rather than passing quietly, and `NATIVE_JOURNEY=required` turns the skip into a failure. What the step probes is `maestro` on PATH, and nothing more — a booted simulator and an installed build belong to the lane that sets the variable, and Maestro reports their absence itself. So `NATIVE_JOURNEY=required` on a lane without a device fails; that is the point, and it is not the same thing as checking for one.

A step that skips is a file nobody reads, which is how `apps/mobile/.maestro/smoke.yaml` came to assert a screen that had been unreachable for two stages. So `pnpm policy` checks the flow wherever it runs: the `appId` must be the identifier this app installs as, every message the flow asserts or taps must be one some catalog in `packages/i18n/messages/` actually carries — every catalog, because which language a flow drives is the flow's own choice — and a flow that asserts nothing at all is rejected. That catches copy drift, which is the common case; it cannot catch a screen becoming unreachable, which only a device can.
